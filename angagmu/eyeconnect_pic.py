...
def move_cursor_smoothly():
    ...
    while True:
        ...
        if 50 < new_x < 300 and 250 < new_y < 400:  # Help
            selected_word = "Help"
        elif 650 < new_x < 800 and 250 < new_y < 400:  # Play
            selected_word = "Play"
        elif 350 < new_x < 500 and 100 < new_y < 270:  # Hungry
            selected_word = "Hungry"
        elif 350 < new_x < 500 and 400 < new_y < 600:  # Bathroom
            selected_word = "Bathroom"
        else:
            selected_word = ""  # No selection if not in any area

        # Add a small delay to control speed
        time.sleep(0.01)  # Increase this delay if it's still too fast
...
def create_word_picker():
    ...
    word_display.pack(pady=10)  # Add padding for better appearance

    # Load images for each word
    help_img = ImageTk.PhotoImage(Image.open("help.png").resize((150, 150)))
    play_img = ImageTk.PhotoImage(Image.open("play.png").resize((150, 150)))
    hungry_img = ImageTk.PhotoImage(Image.open("hungry.png").resize((150, 150)))
    bathroom_img = ImageTk.PhotoImage(Image.open("bathroom.png").resize((150, 150)))

    # Create labels for images at their positions
    labels = {
        "Left": tk.Label(root, image=help_img),
        "Right": tk.Label(root, image=play_img),
        "Top": tk.Label(root, image=hungry_img),
        "Bottom": tk.Label(root, image=bathroom_img),
    }

    # Position images on the screen
    labels["Left"].place(x=50, y=250)
    labels["Right"].place(x=650, y=250)
    labels["Top"].place(x=350, y=120)
    ...
    def highlight_and_display():
        ...
        root.after(100, highlight_and_display)  # Schedule the next update

    # Start the highlight_and_display function
    highlight_and_display()
